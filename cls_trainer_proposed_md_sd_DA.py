...
#%% #################################### Model Trainer Class #################################### 
class ModelTrainerProposed_md_sd_DA():
    def __init__(self, 
    ...
    def train_one_epoch(self):
        ...
        for itera_no, data in enumerate(self.trainLoader):                                   
            ...
            images, images_orig, targets = data
            images = images.to(DEVICE) 
            targets = targets.to(DEVICE)                
            images_orig = images_orig.to(DEVICE)            
            
            with torch.cuda.amp.autocast():
                out = self.model(images, True, targets, images_orig)  
                cls_loss = out['cls_loss'].mean()         
                gfe_loss = out['gfe_loss'].mean()
                consistency_loss = out['consistency_loss'].mean()
                if self.apply_gfe_loss and (self.apply_l_ccr or self.apply_l_scr):
                    batch_loss = cls_loss+consistency_loss+gfe_loss
                elif self.apply_l_ccr or self.apply_l_scr:
                    batch_loss = cls_loss+consistency_loss+gfe_loss
                elif self.apply_gfe_loss:
                    batch_loss = cls_loss+gfe_loss
                else:
                    batch_loss = cls_loss
            ...
            train_info_box.update({'loss':[batch_loss.detach().item(), targets.shape[0]],
                                   ...
                                   'gfe_loss':[gfe_loss.detach().item(), targets.shape[0]],
                                   'consistency_loss':[consistency_loss.detach().item(), targets.shape[0]]
                                   })
